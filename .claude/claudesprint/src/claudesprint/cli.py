...
def init_project(
    ...
    prompt_file = Path(config.prompts_dir) / "PROMPT_init.md"
...
def run_planner(
    ...
    prompt_file = Path(config.prompts_dir) / "PROMPT_plan.md"
    if not prompt_file.exists():
        console.print(f"[red]Error: {prompt_file} not found[/red]")
        raise typer.Exit(1)
    ...
    result = runner.run_prompt(prompt_file, on_output=lambda line: console.print(line), model=model)
